...
use embassy_imxrt::timer::{CTimerPwm, CTimerPwmLengthChannel, TimerChannelNum};
...
async fn main(_spawner: Spawner) {
    ...
    let ctimerlenchannel = CTimerPwmLengthChannel::new(p.CTIMER4_COUNT_CHANNEL2, MicroSeconds(10_000));

    // CTIMER4_MAT3 configuration for PIO0_31
    info!("GPIO0_31 is red LED on rt685-evk");
    let mut ctimer = CTimerPwm::new(p.CTIMER4_COUNT_CHANNEL3, &ctimerlenchannel, p.PIO0_31);

    ctimer.enable(TimerChannelNum::Channel3);
    ...
    loop {
        ...
        ctimer.set_duty(TimerChannelNum::Channel3, duty);
        ...
        for pct in 0..=100u8 {
            ...
            ctimer.set_duty(TimerChannelNum::Channel3, CentiPercent(pct, 0));
        ...
        for pct in 0..=100u8 {
            ...
            ctimer.set_duty(TimerChannelNum::Channel3, CentiPercent(100 - pct, 0));
            Timer::after_millis(100).await;
        }
